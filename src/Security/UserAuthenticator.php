...
class UserAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        /// Ne laisse se connecter que les compte activé !
        if ($user->getActivationToken() != null) {
            throw new CustomUserMessageAuthenticationException('Vous devez activer votre compte avant de vous connecter ! ');
        }
        /// Vérifier la validité du  mot de passe  !
        if ($this->checkCredentials($credentials, $user) == false) {
            throw new CustomUserMessageAuthenticationException('Votre mot de passe est incorrecte ! ');
        }
        if($user->getIsBlocked()==true){
            throw new CustomUserMessageAuthenticationException('Vous êtes bloqué ! ');
        }
